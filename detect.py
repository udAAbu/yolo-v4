...
flags.DEFINE_string('images', './data/images/', 'path to input image')
...
def main(_argv):
    ...
    for count, image_path in enumerate(os.listdir(image_dir), 1):
        original_image = cv2.imread(image_dir + image_path)
        ...
        if FLAGS.crop:
            #crop_path = os.path.join(os.getcwd(), 'detections', 'crop', image_path)
            crop_path = os.path.join(os.getcwd(), 'detections', 'crop')
            try:
                os.mkdir(crop_path)
            except FileExistsError:
                pass
            crop_objects(cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB), pred_bbox, crop_path, allowed_classes, image_path)
        ...
        cv2.imwrite(FLAGS.output + image_path[-13:], image)
